...
List<MemberDTO> list = dao.getMemberlist(choice, search, pageNumber);
...
<body>
	<div class="card shadow ml-3 mr-3"><!-- [CSS] table -->
		<table border="1" class="table table-bordered" id="dataTable"
			...
			<% for (int i = 0; i < list.size(); i++) {
					...
					WeatherLocationDTO wdto = wdao.getState(dto.getLocationCode());
						%><tr>
				<td><input type="checkbox" id="<%= dto.getUserID()%>"
					class="chk_content"></td>
				<td><%= dto.getUserID() %></td>
				<td><%= dto.getUserName() %></td>
				<td><%= dto.getEmail() %></td>
				<td><%= dto.getGradeName()%></td>
				<td><%= dto.getLocationCode()%></td>
				<td><%=wdto.getState() %></td>
...
%></ul>
		...
		<div align="center" >
			<div style="width: 100px; float: left;" class="ml-3 mb-3"><!-- [CSS] class 추가 -->
...
function searchBtn() {
	...
	location.href="Main.jsp?content=admin/UserlList&choice="+choice+"&search="+search; //검색했을 때 검색한 결과만 가지고페이지 갱신해야함
...
function goPage( pageNum ) {
	...
	let url = "Main.jsp?content=admin/UserlList&pageNumber=" + pageNum;
